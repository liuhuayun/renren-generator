...
import io.renren.entity.Greeting;
...
public class GreetingController {  
     ...
     * @param headers 
     */  
    @MessageMapping("/hello") //"hello"为WebSocketConfig类中registerStompEndpoints()方法配置的 消息路径：接受客户端消息
    @SendTo("/topic/greetings")  //客户端订阅消息的路径：发送给客户端消息
    public Greeting greeting(@Header("atytopic") String topic, @Headers Map<String, Object> headers) {  
        System.out.println(headers);
        return new Greeting("you are connect successfully");
    } 
    
    @MessageMapping("/mymsg") //"mymsg"为WebSocketConfig类中registerStompEndpoints()方法配置的消息路径  ：
    @SendTo("/topic/feed")//客户端订阅消息的路径  ：发送给客户端消息
    public Greeting feed(@Header("atytopic") String topic, @Headers Map<String, Object> headers) {  
        System.out.println(headers);
        return new Greeting("you are connect successfully2");
    } 
